...
;(function($) {
   ...
   'use strict'
		...
		function sliderHeight() {
			...
			$('#slideshow').imagesLoaded( function() {
				if ( $(window).width() <= 1024 ){	
					...
					$('.WildBeyond-hero-area, #slideshow').height(slideItemHeight);
				} else {
					$('.WildBeyond-hero-area').css('height', 'auto');
				}
			});
		}
	...
	var videoPopup = function() {
		...
		function closePopup() {
			if ( $('.WildBeyond-video.vid-lightbox .video-overlay').hasClass('popup-show') ) {
			    
				var popup = $('.WildBeyond-video.vid-lightbox .video-overlay.popup-show');
		...
		$('.WildBeyond-video.vid-lightbox .video-overlay').on('click',function () {
			closePopup();
		});

		$('.WildBeyond-video.vid-lightbox').parents('.panel-row-style').css({'z-index': '12', 'overflow': 'visible'});	
